...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    // Get the billId and totalAmount from POST request
    $billId = $_POST['bill_id'];
    $totalAmount = $_POST['total_amount'] * 100; // Convert to paisa

    $secretKey = "Key aa815fe4787c4b209f6576ce7b85a10d";
    
    $payload = json_encode([
        "return_url" => "http://localhost:8080/nea-project/payment_success.php",
    ...
    // If the response is successful, redirect to payment URL
    ...
    header("Location: http://localhost:3000/consumer");
    exit();
}
?>
...
<body>
    <div class="payment-container common-styles">
        ...
        <div class="payment-details data-display-card">
            <p>Bill ID: <span class="data-value"><?php echo htmlspecialchars($_POST['bill_id']); ?></span></p>
            <p>Total Amount: Rs. <span class="data-value"><?php echo number_format($_POST['total_amount'], 2); ?></span></p>

            <button id="pay-button" class="pay-button">Pay with Khalti</button>
        </div>
        <a href="http://localhost:3000/consumer" class="dashboard-button">Back to Dashboard</a>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            var config = {
                publicKey:"4c63bf2fda174161bd201e6f54f1aec2", // Your Khalti public key
                productIdentity: "<?php echo $_POST['billId']; ?>",
                productName: "Electricity Bill Payment",
                productUrl: "http://localhost:8080/nea-project/khalti_payment.php", 
                amount: <?php echo $_POST['total_amount'] * 100; ?>,  // Ensure amount is in paisa
                eventHandler: {
                    onSuccess(payload) {
